 ...
 * typecheck / tests / build), review code style, and check documentation
...
function buildAuditPrompt(workdir, languages, cfg, styleGuide) {
  ...
  parts.push(
    cfg.checkDocs
      ? "3. Check documentation coverage: README present and accurate, public API and signatures documented,"
        + " key modules or functions with obvious missing docs. Flag concrete gaps, not pedantry."
      : "3. (documentation coverage check disabled)",
    "",
    "Constraints:",
...
function renderStatus(lv, cfg) {
  return [
    ...
    `  style: ${cfg.checkStyle ? "on" : "off"} · docs: ${cfg.checkDocs ? "on" : "off"}`,
...
export function apply(ctx, config) {
  ...
  ctx.systemPrompt.section({
    ...
    text:
      ...
      "checks, reviews code style, and checks documentation coverage. For goal-based work, update_goal " +
