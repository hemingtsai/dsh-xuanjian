 ...
 * typecheck / tests / build), review code style, hunt for real logic bugs, and
 * check documentation coverage, then returns a structured verdict. If a
 * required check fails, the failure is handed back to the model so it fixes
 * the code, and the task is not allowed to end until the audit passes (or a
 * configured safety bound is reached).
 *
...
function buildAuditPrompt(workdir, languages, cfg, styleGuide) {
  ...
  parts.push(
    cfg.checkDocs
      ? "4. Check documentation coverage: README present and accurate, public API and signatures documented,"
        + " key modules or functions with obvious missing docs. Flag concrete gaps, not pedantry."
      : "4. (documentation coverage check disabled)",
    "",
    "Constraints:",
    "- READ-ONLY review: run checks and read files, but NEVER write, edit, or delete anything; never commit;",
    "  never install packages; never spawn agents; never call run_audit or any audit-gate mechanism.",
...
async function runAudit(ctx, agent, cfg, signal) {
  ...
  const prompt = buildAuditPrompt(workdir, languages, cfg, styleGuide);
  ...
  try {
    ...
    return [{ name: "audit", run: "llm-audit", required: false, passed: false, output: `audit could not start: ${error?.message ?? String(error)}` }];
  }
  ...
  if (result.stopReason !== "completed" || !isPlainObject(structured) || !Array.isArray(structured.checks)) {
    ...
    return [{ name: "audit", run: "llm-audit", required: false, passed: false, output: `${reason} — not blocking this run; re-audit on the next gate trigger.` }];
  }
  ...
  if (verdicts.length === 0) {
    return [{ name: "audit", run: "llm-audit", required: true, passed: structured.passed === true, output: trimOutput(structured.summary, 4000) }];
  }
  return verdicts;
}
...
function renderStatus(lv, cfg) {
  return [
    ...
    `  style: ${cfg.checkStyle ? "on" : "off"} · logic: ${cfg.checkLogic ? "on" : "off"} · docs: ${cfg.checkDocs ? "on" : "off"}`,
...
export function apply(ctx, config) {
  ...
  ctx.systemPrompt.section({
    ...
    text:
      ...
      "checks, reviews code style, hunts for real logic bugs, and checks documentation coverage. " +
      "For goal-based work, update_goal " +
